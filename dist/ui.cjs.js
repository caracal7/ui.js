...
var ParseError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "ui.js \u2764\uFE0F  brackets parse ERROR";
  }
};
...
function addText(p) {
  ...
  if (start === end && !opt.includeEmpty)
    return;
  const escapers = opt.pairs.reduce((p2, c) => p2.concat(c.split("")), []).map((v) => "".concat(opt.escape).concat(v));
...
function parseBrackets_default(text, options) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (skip && ["{", "}", "(", ")", "[", "]"].includes(c) && parent2.nodeType === "root") {
      ...
      if (ns.length >= nestMax)
        throw new ParseError("over nest max limit. options: { nestMax: '".concat(opt.nestMax, "' }"));
      ...
      if (parent2.nodeType === "root" || opens[c] !== parent2.open)
        throw new ParseError("404 pair '".concat(opens[c], "' :").concat(i));
  ...
  if (insideQuote !== null) {
    throw new ParseError("unclosed quote ".concat(insideQuote, ' near "...').concat(text.substring(p - 20, p + 30), '..."'));
  }
  const parent = safePop(ns);
  if (parent.nodeType !== "root")
    throw new ParseError("404 pair '".concat(parent.open, "' :").concat(parent.pos.start));
...
function mergeAttributes(items) {
  items.filter((i) => i.opts && i.opts.length).forEach((item) => {
    ...
    if (loop) {
      const key = loop.key;
      item.opts2.key = new Function("return ".concat(key))();
    ...
    eventCallbacks.forEach((event) => {
      ...
      if (item.opts2.events[name])
        throw 'ui.js compile error: Duplicate event name "'.concat(name, '"');
      if (event.value[0] === "(")
        console.warn('"'.concat(event.key, '(" is deprecated. Use "').concat(event.key, '{" inside "').concat(item.content.length > 40 ? item.content.substring(0, 40) + "..." : item.content, '" declaration'));
    ...
    eventBinds.forEach((event) => {
      ...
      if (item.opts2.events[name])
        throw 'ui.js compile error: Duplicate event name "'.concat(name, '"');
      item.opts2.events[name] = new Function("return (...args) => this.".concat(event.value, "(...args)"))();
    });
    ...
    eventCallFunctions.forEach((event) => {
      ...
      if (item.opts2.events[name])
        throw 'ui.js compile error: Duplicate event name "'.concat(name, '"');
      item.opts2.events[name] = "".concat(event.value).concat(event.args);
    });
  });
  items.filter((i) => i.opts && i.opts.length).forEach((item) => {
    if (!item.opts2)
    ...
    booleanAttributes.forEach((attr) => {
      if (item.opts2.attrs2[attr.key]) {
        console.warn('ui.js compile warning: direct attibute "'.concat(attr.key, '" override attrs({...').concat(attr.key, '...}) declaration in tag ""!'));
      }
    ...
    jsAttributes.forEach((attr) => {
      ...
      if (item.opts2.attrs2[key]) {
        console.warn('ui.js compile warning: direct attibute "'.concat(key, '" override attrs({...').concat(key, '...}) declaration in tag ""!'));
      }
  ...
  if (a === block && AAA.length) {
    ...
    if (c[0] === "(") {
      if (item.opts.find((x) => x.type === block)) {
        throw "ui.js template parse error: duplicate ".concat(block, '() inside "<').concat(tag, '>" declaration');
      }
      item.opts.push({ type, value: c.slice(1, -1) });
    } else {
      if (c[0] === "{") {
        if (block === "if")
          throw 'ui.js template parse error: expected "if(..." but "if'.concat(c[0], '..." found inside "<').concat(tag, '>"');
        if (item.opts.find((x) => x.type === block)) {
          throw "ui.js template parse error: duplicate ".concat(block, '() inside "<').concat(tag, '>" declaration');
        }
        item.opts.push({ type, value: c.slice(1, -1) });
      } else {
        throw 'ui.js template parse error: expected "'.concat(block, '{..." but "').concat(block).concat(c[0], '..." found inside "<').concat(tag, '>"');
      }
    }
    return true;
  }
      ...
      if (A.nodeType === "bracket") {
        if (item.opts.length) {
          ...
          if (last.type === "jsAttribute") {
            ...
            throw 'ui.js template parse error: unxpected brackets "'.concat(A.content, '" at the end of <').concat(tag, "> attributes");
        } else
          throw 'ui.js template parse error: unxpected brackets "'.concat(A.content, '" at the end of <').concat(tag, "> attributes");
        ...
        do {
          ...
          if (bbb[0] && bbb[0][0] === "=") {
            ...
            if (a[0] === "@") {
              if (['"', "'", "`"].includes(b[0])) {
                throw "ui.js template parse error: quotes around ".concat(b, " is prohibited");
              }
              ...
              if (b.length) {
                ...
                if (!AAA.length) {
                  throw 'ui.js template parse error: unxpected end of attribute "...'.concat(a, '=" inside "<').concat(tag, '>" declaration');
                }
                const c = AAA.shift().content;
                if (c[0] === "{") {
            ...
            if (a === "loop" && AAA.length) {
              ...
              if (c[0] === "(") {
                if (item.opts.find((x) => x.type === "loop")) {
                  throw "ui.js template parse error: duplicate loop".concat(c, ' inside "<').concat(tag, '>" declaration');
                }
                const [value, key] = splitByLast(c.slice(1, -1));
                if (key) {
                  item.opts.push({ type: "loop", key, value });
                } else {
                  throw 'ui.js template parse error: non-keyed loop not supported yet. "loop'.concat(c, '" inside "<').concat(tag, '>"');
                }
              } else {
                throw 'ui.js template parse error: expected "loop(..." but "loop'.concat(c[0], '..." found inside "<').concat(tag, '>" ');
              }
              ...
              if (c[0] === "(") {
                if (item.opts.find((x) => x.type === "text")) {
                  throw 'ui.js template parse error: duplicate text{c} inside "<'.concat(tag, '>" declaration');
                }
                item.opts.push({ type: "text", value: c.slice(1, -1) });
              } else {
                throw 'ui.js template parse error: expected "text(..." but "text{c[0]}..." found inside "<'.concat(tag, '>"');
              }
            } else if (a === "html" && AAA.length) {
              const c = AAA.shift().content;
              if (c[0] === "(") {
                if (item.opts.find((x) => x.type === "html")) {
                  throw 'ui.js template parse error: duplicate html{c} inside "<'.concat(tag, '>" declaration');
                }
                item.opts.push({ type: "html", value: c.slice(1, -1) });
              } else {
                throw 'ui.js template parse error: expected "html(..." but "html{c[0]}..." found inside "<'.concat(tag, '>"');
              }
            } else {
              if (a) {
                if (a[0] === "@") {
...
function renameAlias(tag, aliases = []) {
  ...
  if (tag[0] == "@") {
    ...
    if (alias) {
      ...
      throw "alias <".concat(tag, "> not defined");
    }
  }
  return tag;
}
...
function toTree(items, aliases) {
        ...
        if (openTag) {
          ...
          if (closeTag) {
            if (selfClosingTags_default[closeTag[1]])
              throw "can't close self-closing tag \"</".concat(closeTag[1], '>"');
            ...
            if (!parent2) {
              if (!node.children || node.children[0].tag !== closeTag[1])
                throw "missing the opening tag for </".concat(closeTag[1], ">");
              throw "closed twice tag <".concat(node.children[0].tag, ">");
            } else {
              if (parent2.text)
                throw "children and text() attribute at the same time";
              ...
              if (opening !== closing)
                throw "the opening and closing tags are not the same <".concat(opening, ">...</").concat(closing, ">");
  ...
  if (tags2.length > 1) {
    ...
    throw "Unclosed tag <".concat(tagName(x.tag, aliases), ">");
  }
...
function animateAttr(element, key, v) {
  ...
  if (fn) {
    from = ((_a2 = element._attrs[key]) == null ? void 0 : _a2.last) || 0;
    ...
    if (typeof value === "object" || element._attrsV[key] !== value || Array.isArray(value)) {
      if (isBooleanAttr(tag.tag, key)) {
        ...
        if ((!tag.attrs3 || !tag.attrs3[key]) && typeof value === "object") {
          if (!(value == null ? void 0 : value.skip)) {
            if (element._attrsV[key] !== value.to) {
              ...
              if (value == null ? void 0 : value.disabled) {
                ((_a2 = element._attrs[key]) == null ? void 0 : _a2.cancel) && element._attrs[key].cancel();
          ...
          if (element._attrsV[key] !== value) {
            element._attrsV[key] = value;
            ((_b2 = element._attrs[key]) == null ? void 0 : _b2.cancel) && element._attrs[key].cancel();
            element.setAttribute(key, value);
          }
        }
      }
        ...
        if (typeof value === "object") {
          if (element._styleV[key] !== value.to) {
            if (!(value == null ? void 0 : value.skip)) {
              ...
              if (value == null ? void 0 : value.disabled) {
                ((_a2 = element._styles[key]) == null ? void 0 : _a2.cancel) && element._styles[key].cancel();
...
var makeFn = (item, fn) => new Function("return ({ ".concat(item.context.join(","), " }) => ").concat(item[fn]))();
var makeFn2 = (item, fn) => new Function("return ({ ".concat(item.context.join(","), " }) => ({").concat(item[fn], "})"))();
var makeFn3 = (item, value, value2) => new Function("return (event, { ".concat(item.context.join(","), " }, element) => {setTimeout(()=>this.render(),0);").concat(value, " = event.target.").concat(value2, "} "))();
var makeFn4 = (item, props) => new Function("return async (event, { ".concat(item.context.join(","), " }, element) => ").concat(props))();
var makeFn5 = (item, props) => new Function("return ({ ".concat(item.context.join(","), " }) => ").concat(props.value + (props.args || "")))();
...
function testUniqueBlock(str, blocks) {
  const i = str.match(new RegExp("^\\s*<!(".concat(blocks, ")>\\s*$"), "i"));
  return i ? i[1] : null;
}
function testBlock(str, block) {
  const i = str.match(new RegExp("^\\s*<!".concat(block, "\\s+([^>]*)>\\s*$"), "i"));
  return i ? i[1] : null;
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!test) {
      if (["css", "import", "tag", "renderer"].includes(block)) {
        if (!blocks.template) {
          ...
          if (line.trim().startsWith("//")) {
            ...
            throw 'unexpected "'.concat(line.trim(), '" at line ').concat(i, ". Template started at line ").concat(templateLine);
          }
        }
      }
      blocks[block] += "\n" + line;
      ...
      if (!blocks[test])
        ...
        throw "duplicate <!".concat(test, "> block at line ").concat(i);
      continue;
    }
  }
  return blocks;
...
function parseTagDirective(tag) {
  ...
  if (args.length > 2)
    throw "excess arguments <!tag ".concat(tag.value, "> at line ").concat(tag.line);
  if (args.some((a) => !a))
    throw "empty argument <!tag ".concat(tag.value, "> at line ").concat(tag.line);
  if (args.length === 1) {
    if (args[0][0] === "@")
      throw "alias is present but path is not <!tag ".concat(tag.value, "> at line ").concat(tag.line);
    result.alias = "@" + args[0].split("\\").pop().split("/").pop();
    result.path = args[0];
  } else {
    if (args[0][0] !== "@")
      throw 'missing alias identifier "@" <!tag '.concat(tag.value, "> at line ").concat(tag.line);
    result.alias = args[0];
    result.path = args[1];
  }
  return result;
...
function parseCSSDirective(tag) {
  ...
  if (splitted.length > 1)
    throw "excess parameters <!import ".concat(tag, ">");
  return stripQuotes(tag);
}
function parseRendererDirective(tag) {
  const splitted = splitByChar(tag, " ", true);
  if (splitted.length > 2)
    throw "excess parameters <!renderer ".concat(tag, ">");
  if (splitted.length == 2 && splitted[1] !== "html")
    throw "unknown second parameter <!renderer ".concat(tag, ">");
  return stripQuotes(tag);
}
...
function parseImportDirective(tag) {
  ...
  if (splitted.length < 3)
    throw "not enough parameters <!import ".concat(tag, ">");
  if (splitted.length === 3) {
    if (splitted[1].toLowerCase() !== "from")
      throw "wrong parameters <!import ".concat(tag, ">");
    if (splitted[0].startsWith("{") && splitted[0].endsWith("}")) {
      ...
      if (!vars2.every(validateName))
        throw "invalid variable name <!import ".concat(tag, ">");
      return {
        type: "object",
        import: splitted[0],
...
var Walt = function(exports2) {
  ...
  const add = (d) => "".concat(d[0]).concat(d[1]);
  ...
  var makeParser = curry_1(function(extraGrammar, source) {
    ...
    parser.feed(source);
    invariant_1(parser.results.length === 1, "PANIC - Ambiguous Syntax! Number of productions (".concat(parser.results.length, ")"));
          ...
          [Syntax.UnaryExpression]: (_ignore) => (args, transform) => {
            ...
            switch (unaryNode.value) {
              case "!":
                ...
                return transform([stmt(_a || (_a = __template(["(((", " >> ", ") | ((~", " + 1) >> ", ")) + 1);"])), lhs, shift, lhs, shift), context]);
              case "~":
                const mask = masks[transform([lhs, context]).type];
                return transform([stmt(_b || (_b = __template(["(", " ^ ", ");"])), lhs, mask), context]);
              case "-":
  ...
  function semantics$2({ stmt }) {
    ...
    function arrayOffset(base2, offset) {
      ...
      return offset.Type !== Syntax.Constant || Number(offset.value) ? stmt(_c || (_c = __template(["(", " + (", " << ", "));"])), base2, offset, shift) : stmt(_d || (_d = __template(["(", ");"])), base2);
    }
    ...
    return {
      ...
      [Syntax.Assignment]: (next) => (args, t) => {
        ...
        invariant_1(sanityCheck(subscript), "PANIC - Cannot assign to subscript of ".concat(lhs.value));
        return transform(stmt(_e || (_e = __template(["", ".store(", ", ", ");"])), type, index2, rhs));
      },
      [Syntax.ArraySubscript]: (next) => (args, t) => {
        ...
        return extendNode({ meta: { TYPE_ARRAY } }, transform(stmt(_f || (_f = __template(["", ".load(", ");"])), type, index2)));
      },
      ...
      semantics({ stmt }) {
        return {
          ...
          [Syntax.FunctionCall]: (next) => (args, transform) => {
            ...
            const callMap = {
              dataSize: transform([stmt(_g || (_g = __template(["i32.load(0);"]))), context]),
              grow: _extends({}, id2, {
                value: "grow_memory",
    ...
    push(type, value, debug) {
      ...
      switch (type) {
        ...
        case "varint1": {
          ...
          size = value.length;
          invariant_1(size, "Cannot write a value of size ".concat(size));
          break;
        }
        case "varint32": {
        ...
        default: {
          size = index_16[type];
          invariant_1(size, "Cannot write a value of size ".concat(size, ", type ").concat(type));
        }
      }
    ...
    return {
      semantics: ({ stmt }) => ({
        [Syntax.StaticDeclaration]: (_next) => ([node, context], transform) => {
          ...
          const uid = "__auto_gen_".concat(node.value, "_").concat(count);
          count += 1;
        ...
        [Syntax.ArraySubscript]: (next) => ([node, context], transform) => {
          ...
          return transform([stmt(_h || (_h = __template(["", ".load(", " + (", " << ", "));"])), target.meta.STATIC, target, offset, shift), context]);
        },
  ...
  const getByteOffsetsAndSize = (objectLiteralNode) => {
    ...
    walkNode({
      [Syntax.Pair]: (keyTypePair) => {
        ...
        invariant_1(offsetsByKey[key] == null, "Duplicate key ".concat(key, " not allowed in object type"));
        keyTypeMap[key] = "".concat(lhs.Type === "AddressOf" ? "&" : "").concat(type);
  ...
  const makeStruct = (stmt) => (base2, field) => {
    const unreachable = stmt(_i || (_i = __template(["throw;"])));
    const fatal = {
      load: extendNode({ range: field.range }, stmt(_j || (_j = __template(["i32.load(", ", ", ");"])), unreachable, unreachable)),
      store: (rhs) => extendNode({ range: field.range }, stmt(_k || (_k = __template(["i32.store(", ", ", ");"])), unreachable, rhs)),
      offset: unreachable,
      type: "void"
    };
    ...
    const direct = type[0] === "&";
    const offset = address ? stmt(_l || (_l = __template(["(", " + ", ");"])), base2, address) : stmt(_m || (_m = __template(["(", ");"])), base2);
    ...
    return {
      offset,
      type,
      store: (rhs) => withMeta(stmt(_n || (_n = __template(["", ".store(", ", ", ");"])), type, offset, rhs)),
      load: withMeta(direct ? offset : stmt(_o || (_o = __template(["", ".load(", ");"])), type, offset))
    };
  };
  function Struct() {
    return {
      semantics({ stmt }) {
        ...
        return {
          ...
          [Syntax.ArraySubscript]: (next) => (args, t) => {
            ...
            }, stmt(_p || (_p = __template(["(", " + (", " * sizeof(", ")));"])), base2, offset, parsed.meta.TYPE_ARRAY)), context]);
          }
        };
      }
    };
        ...
        return {
          [Syntax.FunctionCall]: (next) => (args, transform) => {
            ...
            if (id2.Type === Syntax.Access && id2.params[0] && id2.params[0].Type === Syntax.Type) {
              ...
              return extendNode({
                value: "".concat(type.value, ".").concat(method.value),
                type: type.value,
  ...
  function generateErrorString(msg, error, marker2, filename, func2) {
    ...
    return "\n" + Line + "\n" + highlight + " ".concat(error) + "\n" + msg + "\n" + "  at ".concat(func2, " (").concat(filename, ":").concat(line, ":").concat(col, ")");
  }
  const def = {};
  const opcodeMap = [];
  const textMap = {};
  ...
  const parseParams = (node) => {
    ...
    walkNode({
      [Syntax.Pair]: (pair, _) => {
        params.push("".concat(pair.params[0].value, " ").concat(pair.params[1].value));
      },
      [Syntax.Type]: (p) => {
        params.push(p.value);
      }
  ...
  const typedefString = (node) => {
    ...
    return "(type " + node.value + " (func".concat(parseParams(paramsNode)).concat(parseResult(resultNode), "))");
  };
  ...
  const getPrinters = (add2) => ({
      ...
      walkNode({
        [Syntax.Pair]: ({ params }, _) => {
          ...
          if (type.value === "Memory") {
            ...
            add2('(import "'.concat(mod.value, '" "').concat(field, '" (memory ').concat(memory.initial).concat(memory.max ? memory.max : "", "))"));
          } else {
            add2('(import "'.concat(mod.value, '" "').concat(field, '" ').concat(typedefString(type), ")"));
          }
        },
        [Syntax.Identifier]: (missing, _) => {
          const { value } = missing;
          add2('(import "'.concat(mod.value, '" "').concat(value, '" (type ??))'));
        }
      })(nodes);
    },
    ...
    [Syntax.FunctionCall]: ({ value, params }, print) => {
      printFormatted(add2, print, { value: "call ".concat(value), params });
    },
    ...
    [Syntax.Identifier]: (node) => {
      ...
      add2("(get_".concat(scope2, " ").concat(node.value, ")"));
    },
    [Syntax.Constant]: (node) => {
      add2("(".concat(String(node.type), ".const ").concat(node.value, ")"));
    },
    [Syntax.FunctionPointer]: (node) => {
      add2("(".concat(String(node.type), ".table_pointer ").concat(node.value, ")"));
    },
    [Syntax.FunctionDeclaration]: (node, print) => {
      const [params, result, ...rest] = node.params;
      add2("(func ".concat(node.value).concat(parseParams(params)).concat(parseResult(result)), 2);
      rest.forEach(print);
      add2(")", 0, -2);
    },
    ...
    [Syntax.ImmutableDeclaration]: (node, print) => {
      ...
      if (node.type === "Memory") {
        ...
        add2("(memory ".concat(memory.initial).concat(memory.max ? " ".concat(memory.max) : "", ")"));
      } else {
        add2("(".concat(scope2, " ") + node.value + " " + String(node.type), 2, 0);
        node.params.forEach(print);
        add2(")", 0, -2);
      }
    },
    [Syntax.StringLiteral]: (node) => {
      add2('; string "'.concat(node.value, '"'), 0, 0);
      add2("(i32.const ??)", 0, 0);
    },
    [Syntax.Type]: (node) => {
      add2(node.value);
    ...
    [Syntax.Assignment]: (node, print) => {
      ...
      add2("(set_".concat(scope2, " ").concat(target.value), 2);
  ...
  const scopeOperation = curry_1((op, node) => {
    ...
    invariant_1(index2 != null, "Undefined index for scope Operation. Possibly missing metadata. op: ".concat(JSON.stringify(op), " node: ").concat(printNode(node)));
    const kind = local != null ? op + "Local" : op + "Global";
    const params = [Number(index2)];
    return {
      kind: def[kind],
      params,
      debug: "".concat(node.value, "<").concat(node.meta.ALIAS || node.type, ">")
    };
  });
  const getType = (str) => {
    switch (str) {
  ...
  function validate(ast, {
    ...
    walkNode({
        ...
        walkNode({
          ...
          [Syntax.Pair]: (pair, __) => {
            ...
            if (!isBuiltinType(type.value) && types[type.value] == null) {
              ...
              problems.push(generateErrorString("Undefined Type ".concat(type.value), "Invalid Import. ".concat(type.value, " type does not exist"), { start, end }, filename, GLOBAL_LABEL));
            }
          }
        })(importNode);
      },
      ...
      [Syntax.Declaration]: (decl, _validator) => {
        ...
        if (!isBuiltinType(decl.type) && !types[decl.type] && !userTypes[decl.type]) {
          problems.push(generateErrorString("Unknown type used in a declaration, " + '"'.concat(String(decl.type), '"'), "Variables must be assigned with a known type.", { start, end }, filename, GLOBAL_LABEL));
        }
      },
      [Syntax.FunctionDeclaration]: (func2, __) => {
        const functionName = "".concat(func2.value, "()");
        walkNode({
          [Syntax.Declaration]: (node, _validator) => {
            const [start, end] = node.range;
            if (!isBuiltinType(node.type) && !types[node.type] && !userTypes[node.type]) {
              problems.push(generateErrorString("Unknown type used in a declartion, " + '"'.concat(String(node.type), '"'), "Variables must be assigned with a known type.", { start, end }, filename, functionName));
            }
          },
          [Syntax.Assignment]: (node) => {
            ...
            if (isConst) {
              problems.push(generateErrorString("Cannot reassign a const variable ".concat(identifier.value), "const variables cannot be reassigned, use let instead.", { start, end }, filename, functionName));
            }
          },
          [Syntax.ArraySubscript]: (node) => {
            const [target] = node.params;
            const [start, end] = node.range;
            problems.push(generateErrorString("Invalid subscript target", "Expected array type for ".concat(target.value, ", received ").concat(target.type), { start, end }, filename, functionName));
          },
          [Syntax.NativeMethod]: (node, _validator) => {
            ...
            problems.push(generateErrorString("Cannot generate property access", 'Cannot assign "'.concat(rhs.value, '". Key is "').concat(offset.value, '"'), { start, end }, filename, functionName));
          },
          ...
          [Syntax.FunctionCall]: (node, _validator) => {
            if (functions[node.value] == null) {
              const [start, end] = node.range;
              problems.push(generateErrorString("Undefined function reference", "".concat(node.value, " is not defined."), { start, end }, filename, functionName));
            }
          },
          [Syntax.IndirectFunctionCall]: (node, _validator) => {
            ...
            if (!isBuiltinType(identifier.type) && type == null) {
              const [start, end] = node.range;
              problems.push(generateErrorString("Cannot make an indirect call without a valid function type", "".concat(identifier.value, " has type ").concat(String(identifier.type), " which is not defined. Indirect calls must have pre-defined types."), { start, end }, filename, functionName));
            }
          }
        })(func2);
      }
    ...
    if (problemCount > 0) {
      const errorString = problems.reduce((acc, value) => {
        return acc + "\n" + "".concat(value, "\n");
      }, "Cannot generate WebAssembly for ".concat(filename, ". ").concat(problemCount, " problems.\n"));
  ...
  const generateFunctionCall = (node, parent) => {
    ...
    block.push({
      ...
      debug: "".concat(node.value, "<").concat(node.type ? node.type : "void", ">")
    });
    return block;
  };
  ...
  const generateDeclaration = (node, parent) => {
    ...
    if (initNode) {
      ...
      return [...generateExpression(_extends({}, initNode, { type }), parent), {
        kind: def.SetLocal,
        params: [metaIndex],
        debug: "".concat(node.value, "<").concat(String(node.type), ">")
      }];
    }
    return [];
  };
  ...
  const generateAssignment$2 = (node) => {
    ...
    block.push({
      ...
      debug: "".concat(target.value, "<").concat(String(target.meta.ALIAS || target.type), ">")
    });
    return block;
  };
  ...
  const generateTypecast = (node, parent) => {
    ...
    invariant_1(metaTypecast, "Cannot generate typecast for node: ".concat(JSON.stringify(node)));
  ...
  const mapSyntax = curry_1((parent, operand) => {
    ...
    invariant_1(mapping, "Unsupported Syntax Token. ".concat(operand.Type, ' "').concat(operand.value, '"'));
  ...
  function generateType(node) {
    ...
    invariant_1(typeof id2 === "string", "Generator: A type must have a valid string identifier, node: ".concat(JSON.stringify(node)));
  ...
  const generateCode = (func2) => {
    ...
    const block = {
      ...
      debug: "Function ".concat(func2.value)
    };
  ...
  function write(version) {
    return new OutputStream().push(index_12, MAGIC, "\\0asm").push(index_12, version, "version ".concat(version));
  }
  ...
  const encode2 = (payload, { type, init, mutable }) => {
    ...
    switch (type) {
      case I32:
        ...
        payload.push(varint32, init, "value (".concat(init, ")"));
        break;
      case F32:
        payload.push(index_9, def.f32Const.code, def.f32Const.text);
        payload.push(index_3, init, "value (".concat(init, ")"));
        break;
      case F64:
        payload.push(index_9, def.f64Const.code, def.f64Const.text);
        payload.push(index_4, init, "value (".concat(init, ")"));
        break;
      case I64:
        payload.push(index_9, def.i64Const.code, def.i64Const.text);
        payload.push(varint64, init, "value (".concat(init, ")"));
    }
  ...
  const emitType = (stream, { params, result }, index2) => {
    stream.push(varint7, FUNC, "func type (".concat(index2, ")"));
    ...
    if (result) {
      ...
      stream.push(varint7, result, "result type ".concat(getTypeString(result)));
    } else {
  ...
  const emitLocal = (stream, local) => {
    ...
    stream.push(varint7, local.type, "".concat(getTypeString(local.type)));
  };
  const emitFunctionBody = (stream, { locals, code, debug: functionName }) => {
    const body = new OutputStream();
    code.forEach(({ kind, params, debug }) => {
      invariant_1(typeof kind !== "undefined", "Fatal error! Generated undefined opcode. debug code: ".concat(JSON.stringify(debug)));
      body.push(index_9, kind.code, "".concat(kind.text, "  ").concat(debug ? debug : ""));
      params.filter((p) => typeof p !== "undefined").forEach((p) => {
        ...
        body.push(type, p, "".concat(stringType));
      });
    });
  ...
  const emitDataSegment = (stream, segment) => {
    ...
    stream.push(varint32, offset, "segment offset (".concat(offset, ")"));
    stream.push(index_9, def.End.code, "end");
  ...
  const emitModuleName = (name) => {
    ...
    emitString(moduleSubsection, name, "name_len: ".concat(name));
    return moduleSubsection;
  };
  const emitFunctionNames = (names) => {
    const stream = new OutputStream();
    stream.push(varuint32, names.length, "count: ".concat(String(names.length)));
    names.forEach(({ index: index2, name }) => {
      stream.push(varuint32, index2, "index: ".concat(String(index2)));
      emitString(stream, name, "name_len: ".concat(name));
    });
    return stream;
  };
  const emitLocals = (localsMap) => {
    const stream = new OutputStream();
    stream.push(varuint32, localsMap.length, "count: ".concat(String(localsMap.length)));
    localsMap.forEach(({ index: funIndex, locals }) => {
      stream.push(varuint32, funIndex, "function index: ".concat(String(funIndex)));
      stream.push(varuint32, locals.length, "number of params and locals ".concat(locals.length));
  ...
  const _debug = (stream, begin = 0, end) => {
    ...
    return stream.data.slice(begin, end).map(({ type, value, debug }) => {
      const pcString = "".concat(pc.toString(), " 0x").concat(pc.toString(16)).padStart(6, " ").padEnd(stream.data.length.toString().length + 1);
      let valueString;
      if (Array.isArray(value)) {
      ...
      const out = "".concat(pcString, ": ").concat(valueString, " ; ").concat(debug);
...
var baseClassMixin = (superclass) => class baseClass extends superclass {
  ...
  async connectedCallback() {
    if (this.renderer && !this.__rendererHTML && this.__rendererName !== this.parent.__rendererName) {
      throw "ui.js diffferent renderers ".concat(this.__rendererName, " and ").concat(this.parent.__rendererName);
    }
...
var htmlClassMixin = (superclass) => class baseClass extends superclass {
  ...
  emitNative(event, payload) {
    console.warn('this.emitNative() is deprecated. Use this.emit(). "'.concat(this.__src, '"'));
...
var _uiElement = class {
  constructor() {
    ...
    if (!this.document)
      throw 'ui.js: target container "'.concat(this.container, '" not found');
    this.baseInit();
    this.htmlInit();
    this.init && this.init();
...
var _baseElement = class {
  constructor(parent, tag) {
    ...
    this.onReady = new Promise(async (Ready) => {
      var _a2, _b2, _c2;
      if ((_a2 = this.renderer) == null ? void 0 : _a2.onReady)
        await this.renderer.onReady;
      if ((_b2 = this.parent) == null ? void 0 : _b2.onReady)
        await this.parent.onReady;
      if ((_c2 = this.renderer) == null ? void 0 : _c2.onConnect)
...
var includeImports = (imports, nodePath, BASE_URL, _path) => imports.reduce((prev, value) => {
  const path = nodePath ? value.path[0] === "@" ? value.path : "ui.js-bundled-tags:" + (value.path[0] === "/" ? BASE_URL : BASE_URL + _path) + "/" + value.path : encode(value.path, value.src);
  if (value.type === "*") {
...
function generateJS(component, baseClass, { inFunc = false, parent }) {
  var style, walt, __walt;
  style = component.style;
  __walt = "";
  style = Object.values(plugins_default).filter((p) => p.processStyle).reduce((style2, plugin) => plugin.processStyle(style2), component.style || "");
  walt = component.walt ? plugins_default.walt.compileWalt(component.walt) : "undefined";
  __walt = "get __walt() { return ".concat(walt, " }");
  const state = "{".concat((component.state || "").trim(), "}");
  const observedAttributes = baseClass !== "customElement" ? "" : new Function("return 'static get observedAttributes() { return '+JSON.stringify(Object.keys(".concat(state, "))+' }'"))();
  const class_name = className(component.name);
  const i_component = Stringify(parseTemplate(component.template, component.tag || []));
  const container = component.container ? 'get container()         { return "'.concat(component.container, '" }') : "";
  var renderer = "";
  ...
  ;
  const code1 = "\n\n        ".concat(component.static || "", ";\n        class ").concat(class_name, " extends ").concat(baseClass, " {\n            ").concat(observedAttributes, "\n            get nanoapp()           { return ").concat(JSON.stringify(component.nanoapp), " }\n            get initial_state()     { return ").concat(state, " }\n            get initial_component() { return ").concat(i_component, " }\n            get css()               { return `").concat(style, "`; }\n            ").concat(__walt, '\n            get __src()             { return "').concat(component.src, '" }\n            ').concat(renderer, "\n            ").concat(container, "\n            ").concat(component.class || "", "\n        };\n    ");
  switch (baseClass) {
    case "customElement":
      const _customElement = "\n                ".concat(component.import ? includeImports(component.import, component.nodePath, component.BASE_URL, component.path) : "", "\n                if(!customElements.get('").concat(component.name, "')) {\n                    ").concat(code1, "\n                    customElements.define('").concat(component.name, "', ").concat(class_name, ");\n                    window['UIjs'].tags['").concat(component.name, "'] = true;\n                };");
      return inFunc ? ";(async () => {".concat(_customElement, "})();") : _customElement;
    case "uiElement":
      const _uiElement2 = "\n                ".concat(component.import ? includeImports(component.import, component.nodePath, component.BASE_URL, component.path) : "", "\n                ").concat(code1, " return new ").concat(class_name, '("').concat("parentparentparent:" + parent, '");');
      return inFunc ? ";(async () => {".concat(_uiElement2, "})();") : _uiElement2;
    case "baseElement":
      const _baseElement2 = "\n                ".concat(component.import ? includeImports(component.import, component.nodePath, component.BASE_URL, component.path) : "", "\n                ").concat(code1, " ;window['UIjs'].tags['").concat(component.name, "'] = ").concat(class_name, ";");
      return inFunc ? ";(async () => {".concat(_baseElement2, "})();") : _baseElement2;
  }
}
...
async function _fetch(path, nanoapp) {
  if (window.document.location.protocol == "file:") {
    console.error("WARNING!!! ui.js cannot fetch external resources from a 'local' filesystem: \"file://".concat(path, '"'));
...
async function fetchFile(path, nodeFetch, nanoapp) {
  ...
  if (src === false)
    throw "ui.js  \u2764\uFE0F error loading " + path;
...
async function fetchAndCreate(name, src, base = "", nanoapp) {
  ...
  if (source === false)
    throw "ui.js  \u2764\uFE0F error loading " + url;
...
function warnIfNotEmpty(script, attrs) {
  if (script.innerText.trim().length)
    console.warn('ui.js warning: contents of "<script type=ui '.concat(attrs.map((a) => a.name).join(" "), '> will be ignored"'));
}
function warnIfEmpty(script, attrs) {
  if (!script.innerText.trim().length)
    console.warn('ui.js warning: empty "<script type=ui '.concat(attrs.map((a) => a.name).join(" "), '>"'));
}
async function processScripts() {
  ...
  try {
    for (const script of scripts) {
      ...
      if (target) {
        if (attrs.length > 2)
          console.warn('ui.js warning: all other attributes for "<script type=ui target="'.concat(target.value, '"> will be ignored"'));
      ...
      if (attrsWithHyphens.length > 1) {
        throw 'ui.js ERROR: more then one attribute with hyphens "<script '.concat(attrsWithHyphens.map((a) => a.name).join(" "), '>"');
      } else {
        if (attrsWithHyphens.length === 1) {
...
// src/index.js
var VERSION = "0.7.8-dev";
!VERSION.endsWith("-dev") && console.log("ui.js \u2764\uFE0F ".concat(VERSION, " alpha experiment. Make user interfaces great again!"));
var UIjs = {
  VERSION,
  tags: tags_default,
  customElement,
