<p enter('margin-left': { to: '50px' })>
    ...
    <button @click({ this.state.stat = [] }) if(state.stat.length) style(background:'red', position: 'absolute', top: '300px', left: '300px')>
        Clear
    </button>

    <table cellspacing=1 cellpadding=0>
        <tr loop(state.stat as item, i| d => d.date) exit( opacity: { from:1, to: 0, duration: 30+i*30 })>
